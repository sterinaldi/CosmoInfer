...
cpdef double logLikelihood_single_event(list hosts, object event, CosmologicalParameters omega, double m_th, int Ntot, int EMcp = 0, str completeness_file = None):
    ...
    schechter, alpha, Mstar = SchechterMagFunction(-23., -4., h = omega.h)
    N_em = int(Integrate_Schechter(-4., -25., -26., schechter,M_cutoff)*Ntot)
    M = N_em-N
    N_noem = Ntot - N_em

    print(N_em, N, M, N_noem)
    if EMcp:
        N_em   = 1
        M      = 0
        N_noem = 0
...
cdef double ComputeLogLhWithPost(Galaxy gal, object event, CosmologicalParameters omega, double zmin, double zmax, double ramin, double ramax, double decmin, double decmax, double M_cutoff, double m_th = 18., double M_max = -4., double M_min = -23):
...
@cython.nonecheck(False)
@cython.cdivision(True)
cdef double ComputeLogLhNoPost(Galaxy gal, CosmologicalParameters omega, double zmin, double zmax, double M_cutoff, double m_th = 18., double M_max = -4., double M_min = -23.):
...
cdef double ComputeLogLhNoEmission(Galaxy gal, CosmologicalParameters omega, double zmin, double zmax, double M_cutoff, double m_th = 18, double M_max = -4., double M_min = -23):
