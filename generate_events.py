...
file_hosts = './upsidedown_cutoff/hosts.txt'
...
full_catalog = np.genfromtxt('./upsidedown_cutoff/mockcatalog.txt', names = True)

for gal in hosts:
    sys.stdout.write('Event {0} of {1}\r'.format(counter, len(hosts)))
    folder = './upsidedown_cutoff/event_'+str(counter)+'/'
